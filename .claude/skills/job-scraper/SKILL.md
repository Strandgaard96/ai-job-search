...
Location flags for the currently installed Danish portals (per their own SKILL.md docs):
`--location 2` for Storkøbenhavn on `jobbank-search`; `--region HovedstadenOgBornholm` on
`jobnet-search`; `--municipality "København"` on `jobdanmark-search`; `-l "Copenhagen,
Capital Region, Denmark"` on `linkedin-search`; city folded directly into `--query` on
`jobindex-search` (its API has no location parameter).

Record, per portal: which categories were run, which location(s) were passed, how many raw
...
Then present new jobs in a table sorted by fit (high first). When Step 1b skipped
