...
files ready    with fit ratings     (tailored)
                   |                     |
                   v                     v
...
- A LaTeX distribution with `xelatex` (needed for the cover letter, which always uses `cover.cls`/XeLaTeX regardless of CV renderer choice): [TeX Live](https://tug.org/texlive/), [MacTeX](https://tug.org/mactex/), [TinyTeX](https://yihui.org/tinytex/), or [MiKTeX](https://miktex.org/). `lualatex` is only needed if you explicitly opt into the legacy LaTeX CV template (pdflatex often fails on modern MiKTeX installs with `fontawesome5` font-expansion errors on that path). If using a minimal TeX install such as TinyTeX or BasicTeX, install the extra packages listed in [SETUP.md](SETUP.md#minimal-tex-install-tinytexbasictex).
...
│   │   │   ├── 05-cv-templates.md     # CV structure + tailoring rules (RenderCV default, LaTeX legacy)
...
3. **Draft** a tailored CV (`cv/overrides/<company>.yaml` by default, or LaTeX if explicitly requested) and cover letter (always LaTeX)
...
- **PDF verification loop.** A CV or cover letter that "looks fine in the source" can still break once compiled: job titles orphan to the next page, cover letters spill onto page 2, bullet fonts silently fall back to the body font. The `/apply` command compiles and visually inspects every PDF and applies targeted fixes until the layout is clean — relevance-weighted content trims for the RenderCV/Typst CV by default, or `\needspace`/`\enlargethispage`/font-matching wrappers for the LaTeX cover letter (and for the CV if the legacy LaTeX template is in use). This runs automatically on every application.
- **ATS verification on the PDF text layer.** An ATS reads the PDF's embedded text, not the rendered page — and any renderer (RenderCV/Typst or LaTeX) can silently produce PDFs whose text extracts as garbage (icon glyphs where the email should be, interleaved lines from multi-column layouts). `/apply` extracts the compiled CV's text layer with `pdftotext` and verifies contact details, reading order, and the posting's keyword coverage against what a parser actually sees. Honesty rule enforced: a keyword the profile doesn't support is acknowledged as a gap, never stuffed in.
...
| `cv/cv.yaml` | RenderCV master CV data (default renderer) - identity, education, experience, skills, publications, projects |
